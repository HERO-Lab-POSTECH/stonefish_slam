...
class CFAR(object):
    ...
    def calc_WGN_threshold_factor_OS(self):
        ...
        for ratio in np.logspace(-2, 2, 10):
            ...
            if ret.success:
                return ret.x[0]
        # Return CA threshold as fallback (OS CFAR is not used anyway if alg='SOCA')
        print(f"[WARN] OS CFAR threshold calculation failed, using CA threshold as fallback")
        return x0
