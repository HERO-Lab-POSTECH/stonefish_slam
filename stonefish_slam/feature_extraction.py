...
from hero_slam.utils.io import *
from hero_slam.utils.topics import *
from hero_slam.utils.conversions import *
from hero_slam.utils.visualization import apply_custom_colormap
#from bruce_slam.feature import FeatureExtraction
from hero_slam import pcl
...
from hero_slam.CFAR import CFAR
...
class FeatureExtraction(object):
    ...
    def generate_map_xy(self, ping):
        ...
        print(f"_total bearing angle: {ping.bearings[-1] - ping.bearings[0]}")
        print(f"_res: {_res}")
        ...
        #build the meshgrid # 여기가 아직 정확히 이해가 안됨.
    ...
    def publish_sonar_image(self, img):
    ...
    def callback(self, sonar_msg):
        ...
        '''
        print(f"-------------------------------------------------")
        print(f"sonar_msg.fire_msg: {sonar_msg.fire_msg}")
        print(f"sonar_msg.ping_id:{sonar_msg.ping_id}")
        print(f"sonar_msg.part_number:{sonar_msg.part_number}")
        print(f"sonar_msg.start_time:{sonar_msg.start_time}")
        print(f"sonar_msg.bearings:{sonar_msg.bearings}")
        print(f"sonar_msg.range_resolution:{sonar_msg.range_resolution}")
        print(f"sonar_msg.num_ranges:{sonar_msg.num_ranges}")
        print(f"sonar_msg.num_beams:{sonar_msg.num_beams}")
        print(f"sonar_msg.ping.width:{sonar_msg.ping.width}")
        print(f"sonar_msg.ping.height:{sonar_msg.ping.height}")
        ...
        #the image is not compressed, just use the ros numpy package
            ...
            cv_origin_img = self.BridgeInstance.imgmsg_to_cv2(sonar_msg.ping, desired_encoding="passthrough")
            cv2.imshow("sonar_msg.ping", cv_origin_img)
            cv2.waitKey(1)
        ...
        peaks = self.detector.detect(img, self.alg) # img: image, self.alg: cfar algorithm 이름(string)
        peaks &= img > self.threshold # threshold를 넘기는 peak만 남김.
        # to visualize peaks
        # cv2.imshow("peaks", (peaks.astype(np.uint8)) * 255)
        # cv2.waitKey(1)

        self.publish_sonar_image(img)
