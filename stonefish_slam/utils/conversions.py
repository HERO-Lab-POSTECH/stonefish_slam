...
def r2g(ros_msg) -> gtsam.Pose3:
    ...
    if msg_type == "Pose":
        ...
        raise NotImplementedError(
            f"Not implemented from {msg_type} to gtsam"
        )
...
def g2r(gtsam_obj:gtsam.Pose3) -> Pose:
    ...
    if isinstance(gtsam_obj, gtsam.Pose3):
        ...
        # ROS2: Convert to Python float, not numpy
        pose_msg.orientation.x = float(quaternion.x())
        pose_msg.orientation.y = float(quaternion.y())
        pose_msg.orientation.z = float(quaternion.z())
        pose_msg.orientation.w = float(quaternion.w())
        # pose_msg.orientation.y = quaternion.y()
        # pose_msg.orientation.z = quaternion.z()
        # pose_msg.orientation.w = quaternion.w()
        # print(type(quaternion.x()))
        # print(type(quaternion.y()))
        # print(type(quaternion.z()))
        # print(type(quaternion.w()))
...
def r2n(ros_msg: Any) -> np.array:  # OculusPing type
