...
**왜 있는가.** 검출기의 성능은 "검출 결과가 위치 인식에 쓰였다"의 전제가 아니다.
검출을 상수(`class_id`·`score`·bbox 크기 고정)로 박아 두면 factor 가 0 이 됐을 때
원인이 검출 부재·stamp 불일치·ISAM2 실패 셋으로만 좁혀지므로, **소비 경로만**
먼저 분리해 검증할 수 있다.

⚠️ 예전 이 자리에는 "시뮬 씬에 학습된 클래스(sofa) 자산이 없어 진짜 검출로는
보일 수 없다"고 적혀 있었다 — **2026-09-02 실측으로 반증됐다.** 가중치의 클래스는
`sofa` 가 아니라 `class_one` 이고, 기존 tilt10 bag(`bluerov2_infrastructure` 씬)에
그 모델을 돌리면 conf 0.25 에서 246 샘플 중 46 프레임·54 박스가 나온다. 그 검출이
무엇을 뜻하는지는 데이터셋이 이 머신에 없어 아직 모르지만, "검출이 안 난다"는
전제는 사실이 아니다. 진짜 노드로 돌리는 경로는 `docs/RUN_TEST.md` 참조.
