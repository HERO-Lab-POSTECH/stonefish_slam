...
이 페이지는 `slam.yaml`의 2D 점유그리드·3D OctoMap 파라미터와, IWLO 강도가중 업데이트법 전용 파일인 `mapping/method_iwlo.yaml`의 모든 파라미터를 이름·기본값·정의 위치·의미·수정효과로 정리한 레퍼런스다.

매핑 설정은 두 단계로 나뉜다. `slam.yaml`은 2D/3D 공통 파라미터(해상도, voxel 크기, ray 처리 옵션 등)를 담고, 업데이트법별 세부 파라미터는 `config/mapping/method_*.yaml`로 분리되어 있다(`slam.yaml`). 현재 기본 업데이트법은 `iwlo`이며(`slam.yaml`), 그 전용 파라미터는 `method_iwlo.yaml`에 정의된다. 전체 파라미터는 `slam.py:44-154`에서 `declare_parameter`로 선언된다.

## 설정 파일 구조

```mermaid
flowchart TD
    A["slam.yaml<br/>(2D/3D 공통)"] --> B["mapping_2d<br/>점유그리드"]
...
`method_iwlo.yaml`의 log-odds 파라미터는 `slam.yaml`의 공통 값을 덮어쓴다(`method_iwlo.yaml:7` 주석 "overrides slam.yaml").

## 2D 매핑 — `slam.yaml`의 `mapping_2d`
...
| 파라미터 | 기본값 | 정의 위치 | 의미 | 수정 효과 |
|---|---|---|---|---|
| `map_3d_voxel_size` | `0.3` | `slam.yaml` | OctoMap voxel 한 변 크기(meters) | 작게 하면 세밀하지만 voxel 수가 늘어 메모리·연산 비용이 급격히 증가; 크게 하면 거칠지만 가볍다 |
| `min_probability` | `0.7` | `slam.yaml` | 점유 voxel로 판정할 최소 확률 | 높이면 점유 판정이 보수적이 되어 잡음 voxel은 줄지만 약한 점유는 누락 |
| `use_cpp_backend` | `true` | `slam.yaml` | C++ 백엔드(DDA + OpenMP) 사용 | `false`면 순수 Python fallback으로 느려진다 |
| `use_cpp_ray_processor` | `true` | `slam.yaml` | C++ ray processor 백엔드 사용 | `false`면 ray 처리를 Python으로 수행 |
| `use_dda_traversal` | `true` | `slam.yaml` | DDA ray traversal 알고리즘으로 자유 공간 순회 | `false`면 DDA를 사용하지 않는 경로로 전환 |
| `bearing_step` | `4` | `slam.yaml` | N번째 bearing마다 처리(`4` → 512/4=128 bearing) | 작게(예 `1`) 하면 모든 bearing을 처리해 조밀하지만 비용↑; 크게 하면 bearing을 건너뛰어 빠르지만 ray가 성겨진다 |
| `use_range_weighting` | `true` | `slam.yaml` | 지수 범위 가중 사용 | `false`면 거리에 따른 가중 감쇠를 끈다 |
| `lambda_decay` | `0.1` | `slam.yaml` | 범위 가중의 감쇠 계수 | 크게 하면 먼 거리 측정의 가중이 더 빠르게 줄어든다 |
| `enable_propagation` | `true` | `slam.yaml` | bearing 전파(ray 사이 보간) 활성화 | `false`면 ray 사이를 보간하지 않아 빈 틈이 생길 수 있다 |
| `propagation_radius` | `2` | `slam.yaml` | ±2 bearing 전파(±2 → 5 ray 폭) | 크게 하면 더 넓게 전파해 메우지만 번질 수 있다 |
| `propagation_sigma` | `1.5` | `slam.yaml` | 전파 Gaussian 가중치 표준편차 | 크게 하면 전파 가중이 더 완만하게 퍼진다 |
| `enable_gaussian_weighting` | `true` | `slam.yaml` | bearing 기반 갱신에 Gaussian 가중 | `false`면 전파 시 균일 가중을 쓴다 |
| `update_method` | `'iwlo'` | `slam.yaml` | 확률 갱신법 선택. `'log_odds'`, `'weighted_avg'`, `'iwlo'` 중 하나 | 선택값에 따라 대응하는 `method_*.yaml`의 세부 파라미터가 적용된다 |
...
이 값들은 `slam.yaml`의 공통 log-odds 값을 덮어쓴다(`method_iwlo.yaml:7`).
...
`slam.yaml`/`method_iwlo.yaml`로 전달되지 않을 때 C++ 측이 사용하는 헤더 기본값은 YAML 기본값과 다르다. 운영 시에는 YAML 값이 우선이며, 아래는 미전달 시 C++ 내부 기본값이다(`cpp/ray_processor.h`).
...
!!! warning "YAML과 C++ 기본값의 불일치"
    C++ 헤더의 기본값은 YAML이 값을 전달하지 않을 때만 쓰이는 fallback이다. 정상 실행에서는 `slam.yaml`/`method_iwlo.yaml`의 값이 노드를 통해 전달되어 이 차이는 드러나지 않는다. 다만 C++ 경계 동작을 직접 분석할 때는 두 기본값이 다르다는 점을 유념해야 한다.
