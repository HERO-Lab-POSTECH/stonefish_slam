...
- **전역 프레임 = `world_ned`(NED)로 통일**(P4d 결정, 2026-06-24). 근거: Stonefish 시뮬레이터가 전역 프레임을 NED로 발행하며(`core/slam.py:904` 주석 "Simulator provides world_ned → base_link_frd directly"), slam의 모든 출력 메시지가 그 NED 전역 위에서 동작하므로 전역 frame_id를 `world_ned`로 맞추는 것이 sim과 정합한다. 이전엔 SLAMNode가 같은 노드 안에서 `"map"`(2D·pose·traj·cloud)과 `'world_ned'`(3D octomap)를 **혼용**했으나, P4d에서 전역 frame_id를 `world_ned`로 통일했다. (P4d는 single-ROV를 `world_ned`, multi-ROV를 `{rov}_world_ned`로 분기했으나, RViz Fixed Frame `world_ned`와 `{rov}_world_ned` 사이 TF가 없어 pose·traj·cloud·constraint display가 렌더링 실패 → 후속 커밋에서 이 4개 시각화 출력을 **무조건 `world_ned`로** 통일하고 `{rov}_world_ned` 변형을 은퇴시켰다. `{rov}_world_ned`를 소비하는 노드는 RViz 외에 없었다. body 프레임의 `child_frame_id`/source_frame `{rov}_base_link`·`{rov}/base_link_frd`는 TF 트리용이라 보존.) **이는 의도적 REP-103/105 비순응이다** — 수중 로봇 도메인 표준이 NED이고 sim이 NED를 강제하므로 `map`(ENU)을 전역에 쓰지 않기로 결정. TF는 모두 identity(회전 0)라 좌표 변환은 없고 frame_id **이름만** 정합한 것이다.
- **로봇-로컬 TF 체인 `odom→base_link`는 프레임 *이름*만 REP-105를 따르고 데이터는 NED다**(`core/dead_reckoning.py:299/305/318/319`). 이전 판에서 이 체인을 "REP-105 ENU 유지"로 기술했으나 **2026-08-21 소비자 전수 추적으로 반증**됐다: `core/depth.py::pressure_to_depth`가 NED(깊을수록 +)를 반환하고 dead_reckoning이 이를 부호 변경 없이 pose z에 넣어 발행하므로, `odom`·`base_link`라는 이름 아래 흐르는 실제 데이터는 **z-down(NED)**이다. 이름은 REP-105 관례를 빌렸을 뿐 축 방향 규정은 따르지 않는다.
- **왜 부호를 고치지 않았나 (추적 근거)**: ① dead_reckoning은 `/dead_reck/odom`·`/dead_reck/path`·`/dead_reck/key_traj`와 `odom→base_link` TF를 발행하는데, **repo 내 구독자가 0개**다(전수 grep). ② SLAM은 `LOCALIZATION_ODOM_TOPIC = "/bluerov2/odometry"` — 시뮬레이터의 NED odometry를 직접 구독한다(`utils/topics.py:18`의 명시 결정). ③ `launch/slam.launch.py`는 dead_reckoning 노드를 **띄우지 않는다**. ④ rviz 설정에도 `/dead_reck` 참조가 없다. ⑤ 이 체인의 TF는 전부 identity라 어디에도 회전 변환이 없다. 즉 체인 전체가 NED로 자기정합적이고 **부호를 뒤집으면 오히려 발행 깊이가 틀려진다** — 결함은 코드가 아니라 이 문서에 있었다. 실기 sign-off는 `P4_FLAGS.md`로 이월.
- ⚠️ **sim 통합 시**: sim도 전역 `world_ned`(NED)라 전역 프레임은 양 repo가 일치하고, dead_reckoning의 로컬 체인도 실측상 NED이므로 **NED↔ENU 변환 경계는 현재 코드에 존재하지 않는다**. `launch`의 `world_ned→map` static TF는 identity이며, 만약 장차 `map`을 REP-105 ENU로 쓰는 소비자가 생기면 그 지점이 변환 경계가 된다. 출처: [REP 103](https://github.com/ros-infrastructure/rep/blob/master/rep-0103.rst), [REP 105](https://github.com/ros-infrastructure/rep/blob/master/rep-0105.rst).
...
- **mapping standalone 노드는 의도적으로 존재한다 — 메인 `slam_node`의 mapping 토글로 대체되지 않는다**(2026-06-24 전수 정독 확정). `nodes/mapping_2d_standalone_node.py`·`mapping_3d_standalone_node.py`는 `core/mapping_2d.py`·`mapping_3d.py`의 **같은 알고리즘 클래스**(`SonarMapping2D`/`SonarMapping3D`)를 쓰므로 알고리즘 중복은 없으나, **입력 경로가 근본적으로 다르다**:
  - **메인 `slam_node`**: factor graph가 최적화한 키프레임으로 맵을 갱신한다 — `SonarMapping2D.update_global_map_from_slam(keyframes, tf2_buffer, ...)`(`core/slam.py:798`)·`SonarMapping3D.update_map_from_slam(new_keyframes, ...)`(`core/slam.py:820`). pose가 loop closure로 보정된다. `enable_2d_mapping`/`enable_3d_mapping`(`core/slam.py:137-138`)로 켜고 끄며, `slam.launch.py mode:=mapping`은 이를 mapping-only 모드(SSM/NSSM off)로 띄우지만 **여전히 SLAM 콜백 파이프라인**(`slam_callback_integrated`)을 거친다.
